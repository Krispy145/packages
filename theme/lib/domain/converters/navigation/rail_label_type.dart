...
class NavigationRailLabelConverter
    implements JsonConverter<NavigationRailLabelType?, String?> {
  ...
  NavigationRailLabelType? example(NavigationRailLabelType type) {
    final json = const NavigationRailLabelConverter()
        .toJson(_typeToNavigationRailLabel(type));
    final Map<String, dynamic> response = jsonDecode(json!);
    final alignment =
        const NavigationRailLabelConverter().fromJson(response['alignment']);
    AppLogger.print(
        "ALIGNMENT fromJson -> $alignment", [PackageFeatures.converters]);
    final convertedJson = const NavigationRailLabelConverter()
        .toJson(NavigationRailLabelType.selected);
    AppLogger.print(
