...
class GooglePlaceSearchFormFieldStore = _GooglePlaceSearchFormFieldStore
    with _$GooglePlaceSearchFormFieldStore;

/// [_GooglePlaceSearchFormFieldStore] is a class that manages the state of the map feature.
abstract class _GooglePlaceSearchFormFieldStore
    extends BaseFormFieldStore<GooglePlace?> with LoadStateStore, Store {
  ...
  late final GooglePlacesRepository search =
      GooglePlacesRepository(apiKey: googleAPIKey);

  @action
  Future<List<GooglePlace>> searchQuery(String query) async {
    final searchPlace = await search.textSearchGooglePlaces(
      parameters: GoogleTextSearchParameters(query: query),
    );
  ...
  Future<void> onSelectedPlace(GooglePlace? googlePlace) async {
    ...
    final googlePlaceFullDetails =
        await search.getGooglePlaceDetails(placeId: googlePlace.placeId ?? "");
