...
@MappableClass()
class DummyDataResponseModel<T> extends ResponseModel
    with DummyDataResponseModelMappable<T> {
  final int? lastIndex;

  DummyDataResponseModel({this.lastIndex});
}

abstract class PaginatedDummyDataSource<T, Q> extends DummyDataSource<T, Q>
    with Paginated<DummyDataResponseModel<T?>, T, Q> {
  @override
  Future<Pair<RequestResponse, Pair<DummyDataResponseModel<T?>, List<T?>>>>
      getPage({
    ...
    final nextResponse =
        DummyDataResponseModel<T?>(lastIndex: nextIndex + nextData.length - 1);
    if (nextData.isEmpty) {
      return Future.value(
