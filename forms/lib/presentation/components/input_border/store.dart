...
abstract class _InputBorderFormFieldStore extends BaseFormFieldStore<InputBorderModel> with Store {
  ...
  void onTypeChanged(InputBorderType newType) {
    type = newType;
    value = (value ?? const InputBorderModel()).copyWith(type_enum_inputBorderType: newType);
  }

  @action
  void onBorderSideChanged(BorderSideModel? borderSide) {
    value = (value ?? const InputBorderModel()).copyWith(borderSide_borderSide: borderSide);
  }

  @action
  void onBorderRadiusChanged(BorderRadiusModel? borderRadius) {
    value = (value ?? const InputBorderModel()).copyWith(borderRadius_borderRadius: borderRadius);
  }

  @action
  void onGapPaddingChanged(double? gapPadding) {
    value = (value ?? const InputBorderModel()).copyWith(gapPadding_double: gapPadding);
  }
