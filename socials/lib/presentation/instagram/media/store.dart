...
abstract class _InstagramMediaStore
  ...
  Future<Pair<RequestResponse, List<InstagramMediaModel?>>> getUserMedia({
    ...
    try {
      ...
      if (mediaIds.isNotEmpty) {
        ...
        setLoaded();
        AppLogger.print(
          "Current State 1: $currentState - results length: ${results.length}",
  ...
  Future<void> getUserMediaIds({
    ...
    try {
      final _response = await repository.getUserMediaIds(
        userId: user?.id,
        accessTokenModel: user!.longLivedToken ?? user.shortLivedToken!,
        refresh: refresh,
      );
  ...
  Future<void> getMediaDetails(String mediaId) async {
    try {
      final _detail = await repository.getMediaDetails(
        mediaId,
        accessTokenModel: user.longLivedToken ?? user.shortLivedToken!,
      );
