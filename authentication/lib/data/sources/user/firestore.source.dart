...
class FirestoreUserDataSource<T extends UserModel> extends PaginatedFirestoreDataSource<T, UserSearchQueryModel> implements UserDataSource<FirestoreResponseModel<T?>, T> {
  ...
  @override
  Query<Map<String, dynamic>> buildQuery(UserSearchQueryModel query, Query<Map<String, dynamic>> collectionReference) {
    if (query.mustExclude) {
      return collectionReference.where("display_name", isNotEqualTo: query.searchTerm);
    } else {
      return collectionReference.where("display_name", isEqualTo: query.searchTerm);
    }
  }
}
