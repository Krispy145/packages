...
class ContactsDataRepository {
  ...
  Future<RequestResponse> addAllContacts({required ContactsDataSource source, required List<Contact> contactsModels}) async {
    return source.addAll(contactsModels);
  }

  /// [additContact] addits a single [Contact] to the data source.
  Future<RequestResponse> additContact({required ContactsDataSource source, required Contact contactsModel}) async {
    return source.update(contactsModel.id, contactsModel);
  }

  /// [deleteContact] deletes a single [Contact] from the data source.
  Future<RequestResponse> deleteContact({required ContactsDataSource source, required String id}) async {
